                                    ...
                                    <div>
                                        ...
                                        <div class="form-outline form-white mb-4">
                                            <select required id="server_id" name="server_id" class="form-control form-select form-control-lg">
                                                <option value="" disabled selected hidden>Server Selection</option>
                                                @foreach ($servers as $server)
                                                    <option value="{{ $server->server_id }}"> {{ $server->server_id }}: {{$server->server_slug}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        @elseif (sizeof($servers) <= 1)
                                            <select name="server_id" id="server_id" hidden>
                                                @foreach ($servers as $server)
                                                    @if ($loop->count == 1)
                                                        <option value="{{ $server->server_id }}">{{ $server->server_id }}</option>
                                                    @elseif ($loop->count == 0)
                                                        <option value="installscript">Installer</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        @else 
                                            <select class="bg-danger text-danger"name="server_id" id="server_id" disabled>
                                                <option value="null" selected>Problem occured.</option>
                                            </select>
                                        @endif
                                        <div class="form-outline form-white mb-4">
                                        ...
                                        <button href="" class="btn d-block mx-auto mt-5 bg-blurple btn-lg px-5 g-recaptcha" data-sitekey="{{env('GOOGLE_CAPTCHA_KEY')}}" data-callback="onSubmit" type="submit"><i class="fa-brands fa-discord" action="{{ route('LOGIN_DISCORD_SUBMIT') }}"></i> Login via Discord</button>
                                    </div>
                                </div>
                            </div>
                        </div>
